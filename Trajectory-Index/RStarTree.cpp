...
void RTree::create(uintptr_t bf, double ff)
{
...
void RTree::insertData(const Object &obj, uintptr_t desiredLevel, 
    byte *overflowArray)
{
    stack<Node *> path;
    stack<uintptr_t> childPosition;
    uintptr_t id = storage->getRoot();
    Node *node = (Node *)storage->read(id);
    path.push(node);
    childPosition.push(0);

    uintptr_t minChild = 0;
    ...
    while(node->level > desiredLevel)
    {
        ...
        if(node->level == 1)
        {
            ...
            uintptr_t k;
            for(uintptr_t i = 0; i < node->numChildren; ++i)
            {
                EntryValue entry;
            ...
            if (minEnlarge < -std::numeric_limits<double>::epsilon() || minEnlarge > std::numeric_limits<double>::epsilon())
            {
                ...
                for(uintptr_t i = 0; i < k; ++i)
                {
                    ...
                    for(uintptr_t j = 0; j < node->numChildren; ++j)
                    {
...
void RTree::reinsert(Node *node, stack<Node *> path, uintptr_t desiredLevel,
    uintptr_t position, byte *overflowArray)
{
    ...
    // node
    uintptr_t i;
    uintptr_t k = std::floor((double)branchFactor * REINSERT_FACTOR);
    for(i = 0; i < k; ++i)
    {
...
void RTree::split(Node *node, Object &a, Object &b)
{
    ...
    uintptr_t m = std::floor((double)branchFactor * SPLIT_FACTOR);
    uintptr_t splitDistribution = (branchFactor - 1) - (2 * m) + 2;
    ...
    // groups
    uintptr_t lowerPartitionIndex[2];
    uintptr_t upperPartitionIndex[2];
    
    // to determine which values are chosen (lower or upper values)
    uintptr_t sortOrder = 0;
    ...
    // choose split axis
    uintptr_t splitAxis = std::numeric_limits<uintptr_t>::max();
    for(uintptr_t dim = 0; dim < 2; ++dim)
    {
        ...
        for(uintptr_t k = 1; k < splitDistribution; ++k)
        {
            uintptr_t d = m - 1 + k;
            ...
            for(uintptr_t j = d + 1; j < branchFactor; ++j)
            {
...
void RTree::cleanRTree(const RTree &rtree)
{
	


}
void RTree::adjustNode(Node *node, Object &a, vector <EntryValue> &entries, uintptr_t startIndex, uintptr_t endIndex)
{
    // setup mbr for object a
    a.mbr = entries.at(startIndex).object.mbr;
    node->objects[0] = entries.at(startIndex).object;
    node->numChildren = 1;
    for(uintptr_t i = startIndex + 1; i < endIndex; ++i)
    {
...
bool RTree::deleteData(const Object &obj, stack<uintptr_t> &path, uintptr_t id)
{    
    Node *node = (Node *)storage->read(id);
    path.push(node->id);
...
void RTree::condenseTree(Node* node, stack<uintptr_t> &path, stack<uintptr_t> &needReinsertion)
{
    fillFactor = 0.7;
	uintptr_t minLoad = floor(branchFactor * fillFactor);
...
void RTree::adjustNode(Node *node)
{
    ...
    for(uintptr_t i = 1; i < node->numChildren; ++i)
    {
